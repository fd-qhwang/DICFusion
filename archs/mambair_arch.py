...
class VSSBlock(nn.Module):
    ...
    def forward(self, input):
        # input [B, C, H, W]
        B, C, H, W = input.shape  # 输入形状 [B, C, H, W]
        input = input.permute(0, 2, 3, 1).contiguous() # 形状 [B, H, W, C]
        x = self.ln_1(input) 
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        # 跳跃连接中使用 DWConv 1
        x = self.dw_conv_1(input.permute(0, 3, 1, 2).contiguous()) + self.drop_path(self.self_attention(x).permute(0, 3, 1, 2).contiguous())
        # 第二阶段跳跃连接使用 DWConv 2
        x = self.dw_conv_2(x) + self.conv_blk(self.ln_2(x.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous())
        #x = x.permute(0, 3, 1, 2).contiguous()
        return x
